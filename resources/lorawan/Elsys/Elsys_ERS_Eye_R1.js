    ...
    for(i=0;i<data.length;i++){
        ...
        switch(data[i]){
            case TYPE_TEMP: //Temperature
                ...
                temp=bin16dec(temp);
                obj.data.temperature=temp/10;
                i+=2;
            break
            case TYPE_RH: //Humidity
                var rh=(data[i+1]);
                obj.data.humidity=rh;
                i+=1;
            break
            case TYPE_ACC: //Acceleration
                obj.data.x=bin8dec(data[i+1]);
                obj.data.y=bin8dec(data[i+2]);
                obj.data.z=bin8dec(data[i+3]);
                i+=3;
            break
            case TYPE_LIGHT: //Light
                obj.data.light=(data[i+1]<<8)|(data[i+2]);
                i+=2;
            break
            case TYPE_MOTION: //Motion sensor(PIR)
                obj.data.motion=(data[i+1]);
                i+=1;
            break
            case TYPE_CO2: //CO2
                obj.data.co2=(data[i+1]<<8)|(data[i+2]);
                i+=2;
            break
            case TYPE_VDD: //Battery level
                obj.data.vdd=(data[i+1]<<8)|(data[i+2]);
                i+=2;
            break
            case TYPE_ANALOG1: //Analog input 1
                obj.data.analog1=(data[i+1]<<8)|(data[i+2]);
                i+=2;
            break
            case TYPE_GPS: //gps
                obj.data.lat=(data[i+1]<<16)|(data[i+2]<<8)|(data[i+3]);
                obj.data.long=(data[i+4]<<16)|(data[i+5]<<8)|(data[i+6]);
                i+=6;
            break
            case TYPE_PULSE1: //Pulse input 1
                obj.data.pulse1=(data[i+1]<<8)|(data[i+2]);
                i+=2;
            break
            case TYPE_PULSE1_ABS: //Pulse input 1 absolute value
                var pulseAbs=(data[i+1]<<24)|(data[i+2]<<16)|(data[i+3]<<8)|(data[i+4]);
                obj.data.pulseAbs=pulseAbs;
                i+=4;
            break
            case TYPE_EXT_TEMP1: //External temp
                var temp=(data[i+1]<<8)|(data[i+2]);
                temp=bin16dec(temp);
                obj.data.externalTemperature=temp/10;
                i+=2;
            break
            case TYPE_EXT_DIGITAL: //Digital input
                obj.data.digital=(data[i+1]);
                i+=1;
            break
            case TYPE_EXT_DISTANCE: //Distance sensor input 
                obj.data.distance=(data[i+1]<<8)|(data[i+2]);
                i+=2;
            break
            case TYPE_ACC_MOTION: //Acc motion
                obj.data.accMotion=(data[i+1]);
                i+=1;
            break
            case TYPE_IR_TEMP: //IR temperature
                ...
                obj.data.irInternalTemperature=iTemp/10;
                obj.data.irExternalTemperature=eTemp/10;
                i+=4;
            break
            case TYPE_OCCUPANCY: //Body occupancy
                obj.data.occupancy=(data[i+1]);
                i+=1;
            break
            case TYPE_WATERLEAK: //Water leak
                obj.data.waterleak=(data[i+1]);
                i+=1;
            break
            ...
            case TYPE_PRESSURE: //External Pressure
                ...
                obj.data.pressure=temp/1000;
                i+=4;
            break
            case TYPE_SOUND: //Sound
                obj.data.soundPeak=data[i+1];
                obj.data.soundAvg=data[i+2];
                i+=2;
            break
            case TYPE_PULSE2: //Pulse 2
                obj.data.pulse2=(data[i+1]<<8)|(data[i+2]);
                i+=2;
            break
            case TYPE_PULSE2_ABS: //Pulse input 2 absolute value
                obj.data.pulseAbs2=(data[i+1]<<24)|(data[i+2]<<16)|(data[i+3]<<8)|(data[i+4]);
                i+=4;
            break
            case TYPE_ANALOG2: //Analog input 2
                obj.data.analog2=(data[i+1]<<8)|(data[i+2]);
                i+=2;
            break
            case TYPE_EXT_TEMP2: //External temp 2
                var temp=(data[i+1]<<8)|(data[i+2]);
                temp=bin16dec(temp);
                obj.data.externalTemperature2=temp/10;
                i+=2;
            break
            case TYPE_EXT_DIGITAL2: //Digital input 2 
                obj.data.digital2=(data[i+1]); 
                i+=1; 
             break
